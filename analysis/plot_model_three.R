...
# Study 2
# input_file = glue('{basepath}model/results/qualitative_sim_v6-3.csv')
# input_file = glue('{basepath}model/results/hand_sim_3.csv')
input_file = glue('{basepath}model/results/jul2/2-nobias.csv')
# input_file = glue('{basepath}model/results/jul2/2-main.csv')
...
summary(lm(log(RT*1000) ~ ConfDif * scale(totalConfidence), data=aSIMc))
summary(lm(log(RT*1000) ~ conf1 * conf2, data=aSIMc))
summary(lm(log(RT*1000) ~ scale(conf1) * scale(ConfDif), data=aSIMc))
